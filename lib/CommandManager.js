 ...
 * @param {string} [logCommand] - Log-safe stand-in for `command` in EVERY log
 *   preview (timer, timeout, slow, failure) — pass one whenever the command
 *   string carries a secret (e.g. a git clone URL with token userinfo)
...
export const executeCommand = (
  ...
  const timer = createTimer(`executeCommand: ${logCommand.substring(0, 50)}`);
    ...
    const timeoutId = setTimeout(() => {
      if (!completed) {
        ...
        log.task.error('Command execution timeout', {
          command: logCommand.substring(0, 100),
          timeout_ms: timeout,

...
export const CLONE_STRATEGIES = ['clone', 'copy', 'localize'];
...
let cachedRootPool = null;

/**
 * The host's ROOT POOL, discovered live (the pool backing /) — the effective
 * default pool wherever a disk entry omits one. Never assumed: the installer
 * names the pool (rpool, bootPool, …). Cached after first resolution; a host
 * whose / is not ZFS answers 'rpool' as the inert off-platform fallback
 * (creates cannot run there anyway).
 * @returns {Promise<string>} Pool name
 */
export const getRootPool = async () => {
  if (cachedRootPool) {
    return cachedRootPool;
  }
  const result = await executeCommand('zfs list -H -o name /');
  const name = result.success ? result.output.trim().split('/')[0] : '';
  cachedRootPool = name || 'rpool';
  return cachedRootPool;
};

/**
 * The pool a DOWNLOADED template lands on — deterministic from
 * template_sources.local_storage_path, so the strategy matrix applies to
 * not-yet-downloaded boxes too.
 * @returns {string} Pool name
 */
export const templateLandingPool = () => {
  const storagePath = config.getTemplateSources()?.local_storage_path || 'rpool/templates';
  return storagePath.split('/')[0];
};

/**
 * A snapshot's GUID — zfs-native identity: send|recv PRESERVES it and
 * snapshots are immutable, so equal guids mean identical content.
 * @param {string} snapshot - Full snapshot name (dataset@snap)
 * @returns {Promise<string|null>} GUID or null when the snapshot is absent
 */
export const snapshotGuid = async snapshot => {
  const result = await executeCommand(`pfexec zfs get -H -p -o value guid "${snapshot}"`);
  return result.success ? result.output.trim() : null;
};

/**
 * The clone-strategy legality matrix: absent = copy, legal everywhere;
 * explicit clone needs the template's pool (thin clones cannot cross pools);
 * explicit localize needs a DIFFERENT pool (the template already lives on
 * its own).
 * @param {Object} boot - Typed boot entry (type template)
 * @param {string} templatePool - Pool the template lives on (or will land on)
 * @param {string} targetPool - Pool the machine's boot volume targets
 * @returns {string|null} Refusal string or null
 */
export const cloneStrategyError = (boot, templatePool, targetPool) => {
  const strategy = boot.clone_strategy || 'copy';
  if (!CLONE_STRATEGIES.includes(strategy)) {
    return `disks.boot.clone_strategy ${strategy} is not a valid strategy (clone|copy|localize)`;
  }
  if (strategy === 'clone' && templatePool !== targetPool) {
    return `disks.boot.clone_strategy clone — the template lives on pool ${templatePool}; a thin clone cannot cross pools (use copy or localize, or target pool ${templatePool})`;
  }
  if (strategy === 'localize' && templatePool === targetPool) {
    return `disks.boot.clone_strategy localize — the template already lives on pool ${templatePool} (use clone or copy)`;
  }
  return null;
};

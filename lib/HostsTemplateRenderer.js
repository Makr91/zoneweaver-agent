...
import { loadSecretsFromFiles } from './ProvisionerConfigBuilder.js';
...
export const buildTemplateContext = ({
  ...
  const { visibleFields } = computeVisibility(
    configuration,
    properties,
    buildShowIfRoleFlags(roles)
  );
  const answers = properties || {};
  ...
  // SECRETS_* globals (§4) — the package's own secrets.yml/.secrets.yml
  if (version?.root) {
    for (const [key, value] of Object.entries(loadSecretsFromFiles(version.root))) {
      ctx[`SECRETS_${sanitizeVar(key)}`] = value;
    }
  }

...
/**
 * Answer-migration transform (Wave-1 sync consensus; Mark's complexity
 * ceiling: RENAMES ONLY — the feature dies before it grows past this).
 * Reads the TARGET version's optional answer-migrations.yml
 * (`from: {"<old-version>": {renames: {old_field: new_field}}}`) and moves
 * renamed keys: an already-present new key wins, everything else passes
 * verbatim, unlisted source versions transform nothing. Any flow rendering
 * an answers map recorded under a DIFFERENT package version calls this
 * before validateAnswers/render.
 * @param {string} versionRoot - The TARGET version's directory
 * @param {string} fromVersion - The version the answers were recorded under
 * @param {Object} answers - The recorded answers map
 * @returns {Object} A new, migrated answers map
 */
export const MIGRATIONS_FILE = 'answer-migrations.yml';

export const applyAnswerMigrations = (versionRoot, fromVersion, answers) => {
  const source = answers && typeof answers === 'object' ? { ...answers } : {};
  const file = path.join(versionRoot, MIGRATIONS_FILE);
  if (!fs.existsSync(file)) {
    return source;
  }
  let doc;
  try {
    ...
    log.api.warn('Unreadable answer-migrations file', { file, error: error.message });
    return source;
  }
  const renames = doc?.from?.[fromVersion]?.renames;
  if (!renames || typeof renames !== 'object') {
    return source;
  }
  for (const [oldName, newName] of Object.entries(renames)) {
    if (oldName in source) {

 ...
 * @param {string} execPath - Guest executable
 * @param {string[]} args - Arguments
 * @param {number} timeoutMs - Total wait for exit
 * @returns {Promise<{exitcode: number|null, stdout: string, stderr: string}>}
 */
export const guestExecAndWait = async (socketPath, execPath, args, timeoutMs = 120000) => {
...
export const guestFileWrite = async (socketPath, destPath, content) => {
  const opened = await runGuestCommand(
    socketPath,
    'guest-file-open',
    { path: destPath, mode: 'w' },
    5000
  );

...
export const scpSyncFiles = async (
  ...
  let scpCmd = `scp ${auth.sshOptions} -P ${port} -r ${source} ${username}@${ip}:${remoteDir}`;
  if (auth.usePassword) {
...
export const syncFilesFromZone = async (
  ...
  let rsyncCmd = `rsync ${rsyncFlags} --rsync-path='sudo rsync' -e "${sshCmd}" ${username}@${ip}:${source} ${localDir}`;
  if (auth.usePassword) {
    rsyncCmd = `sshpass -p '${auth.password}' ${rsyncCmd}`;
  }
...
export const scpSyncFilesFromZone = async (
  ...
  let scpCmd = `scp ${auth.sshOptions} -P ${port} -r ${username}@${ip}:${source} ${localDir}`;
  if (auth.usePassword) {
    scpCmd = `sshpass -p '${auth.password}' ${scpCmd}`;
  }

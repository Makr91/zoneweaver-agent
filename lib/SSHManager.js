 ...
 * Tier 3 (recovery over zlogin) lives at the wait gate, not here. Returns
 * null when no key file exists anywhere (callers fall through to password
 * auth, then to the agent key path for an honest downstream error).
...
/**
 * Tier-3 RECOVERY (Mark's ruling — both known keys rejected = unknown
 * state): fetch the guest's rotated key over zlogin (the NON-SSH transport
 * this platform has) and land it at the working copy's key path, 0600.
 * Callers retry ONCE after a success; a missing guest file or unreadable
 * zone fails honestly ("re-provision or supply the key" is the caller's
 * wording).
 * @param {string} zoneName - Zone name (the zlogin target)
 * @param {string} username - Guest user owning the key
 * @param {string} destPath - Working-copy key path to land at
 * @param {Function|null} onData - Output sink
 * @returns {Promise<boolean>} Whether a key was fetched and landed
 */
export const fetchGuestKeyViaZlogin = async (zoneName, username, destPath, onData = null) => {
  const result = await executeCommand(
    `pfexec zlogin ${zoneName} cat /home/${username}/.ssh/id_ssh_rsa`,
    60000,
    onData
  );
  if (!result.success || !result.output.includes('PRIVATE KEY')) {
    return false;
  }
  fs.mkdirSync(path.dirname(destPath), { recursive: true });
  fs.writeFileSync(destPath, `${result.output.trim()}\n`, { mode: 0o600 });
  fs.chmodSync(destPath, 0o600);
  log.task.info('Guest SSH key recovered over zlogin', { zone_name: zoneName, dest: destPath });
  return true;
};
...
export const generateSSHKey = async keyPath => {
  ...
  const genResult = await executeCommand(
    `pfexec ssh-keygen -t ed25519 -f ${path} -N "" -C "zoneweaver-agent@provisioning"`
  );

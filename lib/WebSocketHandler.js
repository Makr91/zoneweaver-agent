...
export const handleWebSocketUpgrade = async (request, socket, head, wss) => {
  try {
    ...
    // Agent API v1 (O1): `machines` is the canonical noun; `zones` stays as this
    // Node agent's alias — accept the VNC upgrade under either.
    const vncMatch = url.pathname.match(
      /\/(?:zones|machines)\/(?<zoneName>[^/]+)\/vnc\/websockify/
    );
    if (vncMatch) {

...
import { DOCUMENT_SECTIONS } from './ZoneConfigDocumentStore.js';
import os from 'os';
 ...
 * @param {boolean} options.useBlocking - Use blocking JSON.parse instead of non-blocking parseAsync (default: false)
...
export const getZoneConfig = async (zoneName, options = {}) => {
  ...
  try {
    return await parseAsync(result.output);
  } catch (err) {
    throw new Error(`Failed to parse zone configuration: ${err.message}`);
  }
};
...
export const getAllZoneConfigs = async () => {
  ...
  try {
    ...
    throw new Error(`Failed to parse zone configurations: ${err.message}`);
  }
};

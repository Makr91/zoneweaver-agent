...
/**
 * LEGACY modify-wire add_disks[] totals (create_new entries). The modify
 * family converges to typed entries in the next cross-agent wave (frozen H8
 * deferral) — this function dies with that wave.
 * @param {Array} addDisks - Modify wire add_disks[]
 * @returns {Map<string, number>} Pool name → total requested bytes
 */
export const calculateLegacyAddDisksRequest = addDisks => {
  const perPool = new Map();
  if (!Array.isArray(addDisks)) {
    return perPool;
  }
  for (const disk of addDisks) {
    if (disk?.create_new) {
      const bytes = sizeToBytes(disk.size || '50G');
      if (bytes > 0) {

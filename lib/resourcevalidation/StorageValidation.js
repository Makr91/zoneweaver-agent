...
import { getRootPool } from '../DiskSpec.js';
 ...
 * @returns {Promise<Map<string, number>>} Pool name → total requested bytes
 */
export const calculateStorageRequest = async disks => {
  ...
  if (boot && (boot.type === 'blank' || boot.type === 'template') && boot.size) {
    addToPool(boot.pool || rootPool, sizeToBytes(boot.size));
  }
  ...
  if (Array.isArray(additional)) {
    for (const disk of additional) {
      if (disk?.type === 'blank' && disk.size) {
        addToPool(disk.pool || rootPool, sizeToBytes(disk.size));
      }
    }
  }

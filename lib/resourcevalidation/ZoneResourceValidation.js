...
import {
  ...
  calculateLegacyAddDisksRequest,
  validateStorage,
...
export const validateZoneModificationResources = async (requestBody, zoneName) => {
  ...
  // Storage validation for add_disks (legacy modify wire — converges to
  // typed entries in the next cross-agent wave, frozen H8 deferral)
  if (validationConfig.storage && requestBody.add_disks) {
    const requestedPerPool = calculateLegacyAddDisksRequest(requestBody.add_disks);
    if (requestedPerPool.size > 0) {
      validators.push(validateStorage(requestedPerPool));
    }
  }
